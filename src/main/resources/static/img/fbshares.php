...
$err=array();
	...
	$get_banner="SELECT fbs.*, csu.id as memb_id from `cs_fb_share` as fbs, `cs_users` as csu WHERE csu.id=fbs.member_id and csu.client_id='".$_SESSION['admin_id']."' ";
	
	if(isset($_GET['header_search']) && $_GET['header_search']!='' )		
	{
        $get_banner.= " and (fbs.url like '%".re_db_input($_GET['header_search'])."%' || fbs.uname like '%".re_db_input($_GET['header_search'])."%' )";
	}

	if(isset($_GET['orderby']) && $_GET['orderby']!='' && isset($_GET['order']) && $_GET['order']=="asc") {
		$get_banner.=" ORDER BY fbs.". $_GET['orderby']." ".$_GET['order'];
	}
	else if(isset($_GET['orderby']) && $_GET['orderby']!='') {
		$get_banner.=" ORDER BY fbs.". $_GET['orderby']." desc";
	}
    else {
        $get_banner.=" ORDER BY fbs.id desc";
    }

    
	$extralink = re_get_all_get_params(array('action', 'pageno' ,'msg'));
	
	$href_url="fbshares.php";
...
<div class="content-wrap">
    ...
    <main id="content" class="content" role="main">
        <ol class="breadcrumb">
            <li>YOU ARE HERE</li>
            <li class="active">FB Share</li>
        </ol>
        <h1 class="page-title">FB - <span class="fw-semi-bold">Share</span></h1>
        <div class="row">
            <div class="col-md-12">
                <section class="widget">
                    <header>
                        <h5>FB Share <span class="fw-semi-bold"> List</span></h5>
                                ...
                                <?php if(re_db_num_rows($res_banner)>0)
                                {
                                    ...
                                    {	 
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo"<a href='user_profile_data.php?mid=".$row_banner['memb_id']."' target='_blank'>".re_db_output($row_banner['uname'])."</a>"; ?></td>
